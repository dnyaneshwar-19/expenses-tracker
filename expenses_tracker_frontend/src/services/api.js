...
export const userAPI = {
  ...
  updatePreferences: (id, preferences) => api.put(`/users/${id}/preferences`, preferences),
  uploadProfilePhoto: (id, formData) => api.post(`/users/${id}/upload-photo`, formData, {
    headers: {
...
export const expenseAPI = {
  ...
  filterByCategory: (category) => api.get(`/expenses/filter/category?category=${category}`),
  filterByPaymentMethod: (method) => api.get(`/expenses/filter/payment-method?paymentMethod=${method}`),
  filterByDateRange: (startDate, endDate) => api.get(`/expenses/filter/date-range?startDate=${startDate}&endDate=${endDate}`),
  filterByTypeAndCategory: (type, category) => api.get(`/expenses/filter/type-category?expenseType=${type}&category=${category}`),
};
...
export const budgetAPI = {
  ...
  getActiveBudget: (userId, category) => api.get(`/budgets/active/user/${userId}/category/${category}`),
  getTotalSpending: (userId, category, startDate, endDate) => 
    api.get(`/budgets/spending/user/${userId}/category/${category}?startDate=${startDate}&endDate=${endDate}`),
  checkBudgetAlerts: (userId) => api.post(`/budgets/check-alerts/user/${userId}`),
};
...
export const notificationAPI = {
  ...
  getUnreadNotifications: (userId) => api.get(`/notifications/${userId}/unread`),
  markAsRead: (notificationId) => api.post(`/notifications/${notificationId}/mark-read`),
  markAsUnread: (notificationId) => api.post(`/notifications/${notificationId}/mark-unread`),
  markAllAsRead: (userId) => api.post(`/notifications/${userId}/mark-all-read`),
  deleteNotification: (notificationId) => api.delete(`/notifications/${notificationId}`),
  deleteAllNotifications: (userId) => api.delete(`/notifications/user/${userId}/delete-all`),
};
...
export const reportAPI = {
  ...
  generateUserReport: (userId, format) => 
    api.get(`/reports/user/${userId}?format=${format}`, { responseType: 'blob' }),
};
